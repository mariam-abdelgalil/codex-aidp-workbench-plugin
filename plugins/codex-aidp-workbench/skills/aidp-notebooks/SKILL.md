...
> Resolve that plugin from the active Codex skills list, `~/.codex/skills`, or installed plugin paths such as
> `~/.claude/plugins/**/oracle-ai-data-platform-workbench-spark-connectors`. Its
> `oracle_ai_data_platform_connectors` helper package is installed once via that plugin's own
> **`aidp-connectors-bootstrap`** skill, which pushes the package to `/Workspace/Shared` and runs a sanity
> import; if the automated path cannot reach the instance, upload it manually. This skill then just authors
> + runs the notebook; for multi-source joins see `aidp-federate`.
