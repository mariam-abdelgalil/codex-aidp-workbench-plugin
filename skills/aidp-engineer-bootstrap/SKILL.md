...
> **Resolve `$PLUGIN_DIR` first** — the helper must be called by absolute path, and the install location
> depends on the host agent. In Codex, prefer the discovered skill-root copy:
> `~/.codex/skills/scripts/aidp_sql.py` or the sibling `../../scripts/aidp_sql.py` from any installed
> `aidp-*` skill. In Claude, use `CLAUDE_PLUGIN_ROOT` when present or the install path from the plugin
> manager. Set `PLUGIN_DIR` to the directory that contains `scripts/aidp_sql.py`; every `aidp_sql.py`
> example in these skills uses `"$PLUGIN_DIR/scripts/aidp_sql.py"`. If no plugin root is available, resolve
> the file directly with filesystem search tools and derive `PLUGIN_DIR` from its parent.
...
<!-- Optional accelerator: if an `aidp`-style MCP server happens to be configured and connected, you MAY use
its tools as a convenience. It is NOT required and the skill never assumes it. Everything above works with
only the oci CLI + a DEFAULT api_key or session-token profile. -->
