...
- **Connecting to an external source** (Oracle ADB/ExaCS, Fusion, EPM, Essbase, Snowflake, S3, Kafka, …) —
  ...
  **`oracle-ai-data-platform-workbench-spark-connectors`** plugin: use its `aidp-<source>` skill for the
  connection recipe — **do NOT hand-roll the connection.** In Codex, first check whether the connector
  skills are discoverable in the active skills list or under `~/.codex/skills`; otherwise search the user's
  installed plugin locations, including `~/.claude/plugins/**/oracle-ai-data-platform-workbench-spark-connectors`.
  Its `oracle_ai_data_platform_connectors` helper package is installed once via that plugin's
  **`aidp-connectors-bootstrap`** skill (pushes it to `/Workspace/Shared`; if the automated path cannot reach
  the instance, upload manually). A single external source needs only that connector skill (then author/run
  via `aidp-notebooks`); `aidp-federate` composes several for cross-source joins.
