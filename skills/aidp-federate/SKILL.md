...
- **Also the entry point for a SINGLE external source** ("a notebook that connects to Fusion / EPM / ADB / …"):
  ...
  skill just adds the join. **Never hand-roll the source connection.** Resolve the spark-connectors plugin
  from the active Codex skills list, `~/.codex/skills`, or the user's plugin install paths such as
  `~/.claude/plugins/**/oracle-ai-data-platform-workbench-spark-connectors`. Run its
  **`aidp-connectors-bootstrap`** skill once to push the `oracle_ai_data_platform_connectors` helper package
  to `/Workspace/Shared`; if the automated path cannot reach the instance, upload the package manually.
